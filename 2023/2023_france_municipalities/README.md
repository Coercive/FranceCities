# France Cities - Process Explanation
...
foreach ($towns as $town) {
    ...
    if(!$coordinate) {
        ...
        elseif($town['TYPE'] === 'ARR') {
            error_log(print_r('Type ARR, coordinate not found', true));
        }
    ...
    if(!$coordinate) {
        error_log(print_r("Coordinate not found : $refInsee - $town[NORMALIZED_NAME]", true));
    }
    ...
    if($coordinate && $coordinate['GEOMETRY_GPS']) {
        ...
        foreach ($gps as $gp) {
            ...
            elseif($json['type'] === 'MultiPolygon') {
                ...
                throw new \Exception("ERROR UNKONW : $json[type]");
            }
